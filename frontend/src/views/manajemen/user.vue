...
async function fetchUnlinkedDokter() {
  try {
...
const totalPages = computed(() => Math.ceil(daftarUser.value.length / itemsPerPage))
const paginatedData = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage
  return daftarUser.value.slice(start, start + itemsPerPage)
})
  ...
  <div class="max-w-6xl mx-auto mt-10 p-4 relative">
    <div class="w-full bg-white p-6 border rounded-lg shadow-md">
      <div class="flex justify-between items-center mb-4 border-b pb-4">
        ...
        <button class="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition transform shadow-md" @click="openAddModal">
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              <tr v-else-if="daftarUser.length === 0">
                <td colspan="5" class="py-12 text-center text-gray-400">
                  Belum ada data pegawai terdaftar.
                </td>
              </tr>
              <template v-else>
                <tr v-for="(user, index) in paginatedData" :key="user.idusers" class="hover:bg-indigo-50/50 transition border-b border-gray-50 last:border-0">
                  ...
                  <td class="px-5 py-4 align-middle">
                    <div class="flex justify-center gap-2">
                      <button class="bg-blue-500 text-white p-1.5 rounded hover:bg-blue-600 shadow-sm transition" title="Edit User" @click="handleEditUser(user)">
                      ...
                      <button class="bg-red-500 text-white p-1.5 rounded hover:bg-red-600 shadow-sm transition" title="Hapus User" @click="openConfirmDialog(user.idusers)">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <div v-if="totalPages > 1" class="flex justify-between items-center mt-6">
          <p class="text-sm text-gray-600">
            ...
            {{ Math.min(currentPage * itemsPerPage, daftarUser.length) }} dari {{ daftarUser.length }} data
          </p>
          <UPagination v-model:page="currentPage" active-color="primary" :total="daftarUser.length" />
        </div>
      </div>
          ...
          <form class="grid grid-cols-2 gap-5" @submit.prevent="handleSubmitUser">
            ...
            <div v-if="form.role === 'dokter' && !isEditMode" class="flex flex-col col-span-2">
              ...
              <p class="text-xs text-gray-400 mt-1 italic">
                Hanya menampilkan dokter yang belum terhubung ke akun
              </p>
            </div>
