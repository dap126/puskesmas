...
router.post('/', verifyToken, (req, res) => {
    ...
    const sql = `INSERT INTO rekam_medis 
        (tgl_periksa, keluhan, tinggi_badan, berat_bedan, tekanan_darah, diagnosa, pasien_idpasien, dokter_id_dokter) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;

    const values = [tgl_periksa, keluhan, tinggi_badan, berat_bedan, tekanan_darah, diagnosa, pasien_idpasien, dokter_id_dokter];
...
router.put('/:id', verifyToken, (req, res) => {
    ...
    const sql = `UPDATE rekam_medis 
                 SET keluhan=?, tinggi_badan=?, berat_bedan=?, tekanan_darah=?, diagnosa=? 
                 WHERE id_rm=?`;

    db.query(sql, [keluhan, tinggi_badan, berat_bedan, tekanan_darah, diagnosa, id], (err, result) => {
        if (err) return res.status(500).json({ error: err.message });
